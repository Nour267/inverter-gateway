 ...
 * every 1 s combine + pack a TELEMETRY frame. Single thread, event loop with poll().
 *
 * Usage: ./build/gateway [--bus vcan0]
 * Step 1 of M4: the frame is printed. Step 2 will send it over TCP. */
...
static struct {
    ...
    unsigned long telemetry_built;    /* TELEMETRY frames built */
} cnt;
...
static void print_counters(void)
{
    log_msg("INFO", "counters: can_rx=%lu can_bad_len=%lu can_unknown_id=%lu "
            "can_out_of_range=%lu telemetry=%lu",
            cnt.can_rx, cnt.can_bad_len, cnt.can_unknown_id,
            cnt.can_out_of_range, cnt.telemetry_built);
}
 ...
 *       If either is missing or older than 3 s, send nothing: old data is never sent as new.
 * In:   now = the current time from now_s()
 * Out:  nothing returned; for now the frame is printed (M4 step 2: sent over TCP) */
static void send_telemetry(double now)
{
    ...
    /* 3. For now: show it */
    printf("TELEMETRY seq=%u  state=%u  %.1f V  %.2f A  %u W  %.1f V AC  %.1f C  |",
           (unsigned)(seq - 1), (unsigned)t.state, t.dc_voltage / 10.0, t.dc_current / 100.0,
           (unsigned)t.ac_power, t.ac_voltage / 10.0, t.temperature / 10.0);
    for (int i = 0; i < n; i++) {
        printf(" %02X", frame[i]);
    }
    printf("\n");
 ...
 * In:   command-line options: [--bus vcan0]
...
int main(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--bus") == 0 && i + 1 < argc) {
            ...
            fprintf(stderr, "usage: %s [--bus vcan0]\n", argv[0]);
            return 1;
        }
    }
    ...
    log_msg("INFO", "gateway started on %s. Ctrl+C to stop.", bus_name);

    double last_telemetry = now_s();
    double last_counters  = now_s();

    while (running) {
        /* 1. Sleep until a CAN frame arrives, or 100 ms pass */
